...
const adminCheck = (req, res, next) => {
  if (req.user != undefined) {
    if (req.user.is_admin == true) {
      ...
      res.json({ response: { resultCode: 0, message: "관리자만 접근 가능합니다."  } });
    }
  }
  else {
...
const saveImages = async(req, res, next) => {
  ...
  fs.writeFile(`${uploadFolder}/${newFilename}`, req.file.buffer, 'binary', function(err) {
    if (err) {
      console.log(err);
       res.json({ response: { resultCode: 0, message: "파일을 저장하는 중 오류가 발생했습니다." } });
    }
  });
  req.body.image = json;
...
router.put('/:num', authenticateUser, adminCheck, uploadImages, saveImages, getResult, function(req, res, next) { //수정
  getConnection((connection) => {
    connection.beginTransaction();
    connection.query("update stock set name = ?, description = ?, brand = ?, price = ?, stock = ?, uploader = ? where num = ?", [req.body.title, req.body.content, req.body.brand, req.body.price, req.body.stock, req.user.id, req.params.num], function(err, rows, fields) {
      if (err) {
        console.log(err);
        ...
        if (req.file && req.body.image) {
          ...
          var a = connection.query("update images set img_src = ?, img_original = ?, img_name = ?, img_ext = ?, board = ?, board_num = ? where board_num = ?", [req.body.image.img_src, req.body.image.img_original, req.body.image.img_name, req.body.image.img_ext, "stock", req.params.num, req.params.num], function(err, rows, fields) {
            if (err) {
              console.log(err);
              res.json({ response: { resultCode: 0, message: "이미지를 수정하는 중에 오류가 발생하였습니다." } });
              connection.release();
              connection.rollback();
            }
            else {
