...
const adminCheck = (req, res, next) => {
  if (req.user != undefined) {
    ...
    res.json({ response: { resultCode: 0, message: "로그인을 확인해주세요" } });
  }
};
  ...
  getConnection((connection) => {
    connection.query("insert into cart(userid, stocknum, count) values (?, ?, ?) ", [req.user.id, req.body.stocknum, req.body.quantity], function(err, rows, fields) {
      if (err) {
        ...
        res.status(200);
        res.json({ response: { resultCode: 1, message: "장바구니 추가가 완료되었습니다." } });
      }
      connection.release();
    });
    //connection.release();
  ...
  getConnection((connection) => {
    ...
    var a = connection.query(sql, arr, function(err, rows, fields) {
      if (err) {
        console.log(err);
        res.status(500)
        res.json({ response: { resultCode: 0, message: "추가하는 중에 오류가 발생하였습니다." } });
      }
      else {
        res.status(200);
  ...
  getConnection((connection) => {
    ...
    connection.query("delete from stock where num = ?", [req.params.num], function(err, rows, fields) {
      if (err) {
        console.log(err);
        res.status(500);
        res.json({ response: { resultCode: 0, message: "글을 삭제하는 중에 오류가 발생하였습니다." } });
        connection.rollback();
        connection.release();
      }
      else {
        ...
        var a = connection.query("delete from images where board = ? and board_num = ?", ["stock", req.params.num], function(err, rows, fields) {
          if (err) {
            console.log(err);
            res.status(500)
            res.json({ response: { resultCode: 0, message: "이미지를 삭제하는 중에 오류가 발생하였습니다." } });
            connection.rollback();
            connection.release();
          }
          else {
