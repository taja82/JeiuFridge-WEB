...
router.post('/login', function(req, res, next) {
  ...
  passport.authenticate('local', { failureFlash: true }, function(err, user, info) {
    if (err)
      return next(err);
    if (!user)
      return res.status(400).json({ SERVER_RESPONSE: 0, SERVER_MESSAGE: "로그인 정보가 일치하지 않습니다." });
    req.logIn(user, function(err) {
      ...
      if (!err) {
        ...
        return res.json({ SERVER_RESPONSE: 1, SERVER_MESSAGE: "로그인", session: req.user });
      }
    });
  })(req, res, next);
});
...
router.get('/idfind', function(req, res, next) {
  res.render('users/idfind');
});
  ...
  getConnection((connection) => {
    var a = connection.query('select id from user where email = ? ', [req.body.email], function(err, rows, fields) {
      if (err) {
        console.log(err);
        res.status(500);
        res.json({ resultCode: 0, message: "아이디 찾는 중에 오류가 발생했습니다" });
      }
      else {
        res.status(200);
        //console.log(rows.insertId);
...
router.get('/pwfind', function(req,res,next) {
  res.render('users/pwfind');
});
    ...
    getConnection((connection) => {
      var a = connection.query('select email, id from user where id = ?', [req.body.id], function(err, rows, fields) {
        if (err) {
          console.log(err);
          res.status(500);
          res.send({ response: { resultCode: 0, message: "이메일 존재 여부를 찾는 도중 문제가 발생하였습니다." } });
        }
        else {
          if (rows.length == 0) {
            res.status(400).send({ resultCode: 0, message: "입력하신 아이디와 일치하는 정보가 없습니다." });
          }
          else {
              ...
              var a = connection.query('update user set password = ? where id = ?', [hash, rows[0].id], function(err, rowss, fields) {
                if (err) {
                  ...
                  transporter.sendMail(emailOptions, function(error, info) {
                    if (error) {
                      console.log(error);
                      res.status(500).send({ resultCode: 0, message: "이메일을 보내는 동안 문제가 발생하였습니다." });
                    }
                    else {
                      res.status(200).send({ resultCode: 0, message: "전송완료", email: rows[0].email });
                    }
                  }); //전송*/
                }
              });
...
router.get('/authcheck', authenticateUser, function(req, res, next) {
  res.status(200);
  res.send({ resultCode: 1, message: "접속 성공" });
...
router.post('/userinfo', authenticateUser, function(req, res, next) {
  getConnection((connection) => {
    var a = connection.query('select num, student_id, id, email, name, password from user where id = ?', [req.session.passport.user], function(err, rows, fields) {
      if (err) {
        console.log(err);
        res.status(500);
        res.send({ resultCode: 0, message: "유저 정보를 보여주는데 문제가 발생하였습니다." });
      }
      else {
        if (rows.length != 0) {
          if (!bcrypt.compareSync(req.body.password, rows[0].password)) {
            ...
            res.render('users/userinfo', { result: rows[0] });
          }
        }
        else {
          res.render('er', { errmsg: "정보가 일치하지 않습니다" });
...
router.put('/userinfo/:num', authenticateUser, checkme, function(req, res, next) {
  var salt = bcrypt.genSaltSync(10);
  var hash = bcrypt.hashSync(req.body.password, salt);
  getConnection((connection) => {
    var a = connection.query('update user set student_id = ?, email = ?, name = ?, password = ? where num = ?', [req.body.student_id, req.body.email, req.body.name, hash, req.body.num], function(err, rows, fields) {
      if (err) {
        console.log(err);
        res.status(500);
        res.render('er', { errmsg: "유저 정보를 보여주는데 문제가 발생하였습니다." });
      }
      else {
        res.render('redirect', { msg: "수정이 완료되었습니다.", redirectUrl: "/" });
      }
    });
    connection.release();
  });
  ...
  getConnection((connection) => {
    var a = connection.query('select student_id, id, name, is_admin from user where id = ?', [req.session.passport.user], function(err, rows, fields) {
      if (err) {
        ...
        if (rows.length == 0) {
          res.status(200).send({ resultCode: 0, message: "아이디와 일치하는 정보가 없습니다." });
        }
        else {
          res.status(200);
          res.send({
  ...
  getConnection((connection) => {
    var a = connection.query('select id from user where id = ?', [req.params.username], function(err, rows, fields) {
      if (err) {
        ...
        if (rows.length == 0) {
          res.status(200).send({ response: { resultCode: 1, message: "이 아이디는 사용가능합니다." }, params: req.params, result: rows[0] });
        }
        else {
          res.status(200);
          res.status(500).send({ resultCode: 0, message: "이미 같은 아이디가 존재합니다." });
        }
      }
    });
    connection.release();
